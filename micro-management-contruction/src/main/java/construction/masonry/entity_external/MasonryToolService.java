...
public class MasonryToolService {
    ...
    public void saveAll(List<ToolDTO> dtos, String phaseId) {
        ...
        for (ToolDTO dto : dtos) {
            MasonryTool entity = mapToEntity(dto);
            entity.setId(dto.getId() != null && !dto.getId().isBlank() ? dto.getId() : UUID.randomUUID().toString());
            entity.setPhaseId(phaseId);
            entity.setMasonry(masonry);

            System.out.println("Persistindo ferramenta Masonry: " + entity.getId() + " - " + entity.getName());
            repository.persist(entity);
        }
    }

    private MasonryTool mapToEntity(ToolDTO dto) {
