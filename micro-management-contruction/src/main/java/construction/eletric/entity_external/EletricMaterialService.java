...
public class EletricMaterialService {
    ...
    private EletricMaterial mapToEntity(MaterialDTO dto, String phaseId, Eletric eletric) {
    ...
    @Transactional
    public void saveAll(List<MaterialDTO> dtos, String phaseId, Eletric eletric) {
        if (dtos == null || dtos.isEmpty()) return;

        List<EletricMaterial> entities = dtos.stream()
            .map(dto -> mapToEntity(dto, phaseId, eletric))
            .collect(Collectors.toList());

        EletricMaterial.persist(entities);
    }
}
