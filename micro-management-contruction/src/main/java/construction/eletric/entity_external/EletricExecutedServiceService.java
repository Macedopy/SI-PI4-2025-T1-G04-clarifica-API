...
@ApplicationScoped
public class EletricExecutedServiceService {
    
    @Inject
    EletricExecutedServiceRepository repository;

    private EletricExecutedService mapToEntity(ExecutedServiceDTO dto, String phaseId, Eletric eletric) {
        EletricExecutedService entity = new EletricExecutedService();
        ...
        entity.setName(
            dto.getName() != null && !dto.getName().trim().isEmpty() 
                ? dto.getName().trim() 
                : "Serviço Elétrico sem nome"
        );

        entity.setTeam(
            dto.getTeam() != null && !dto.getTeam().trim().isEmpty()
                ? dto.getTeam().trim()
                : "Equipe Elétrica Padrão"
        );
    ...
    @Transactional
    public void saveAll(List<ExecutedServiceDTO> dtos, String phaseId, Eletric eletric) {
        if (dtos == null || dtos.isEmpty()) return;

        List<EletricExecutedService> entities = dtos.stream()
            .map(dto -> mapToEntity(dto, phaseId, eletric))
            .collect(Collectors.toList());
        
        EletricExecutedService.persist(entities);
    }
}
